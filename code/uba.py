...
def get_features_by_ngram_iforest():
    global max_features
    global  index
    x_arr,y=get_cmdlines()
    x=[]
...
def do_iForest(X,Y):
    ...
    clf = IsolationForest(max_samples=150,max_features=max_features)
...
if __name__ == "__main__":
    ...
    do_rnn_wordbag(x_train, x_test, y_train, y_test)
    """
    print "Isolation Forest and wordseq"
    max_features = 100
    #index=50
    print "max_features=%d" % (max_features)

    X,Y = get_features_by_ngram_iforest()
